...
| **兼容性** | ★★★★★ 纯增量特性，不影响已有代码 |
| **复杂度** | ★★★☆☆ 接口限制需要理解，但规则清晰 |
| **落地时间** | Go 1.27（2026 年 8 月已发布） |

Go 泛型方法的到来，标志着 Go 泛型体系从"可用"走向"好用"。虽然接口方法集的限制依然存在，但对于绝大多数容器操作、流式 API 和中间件场景，泛型方法已经足够强大。如果你正在维护一个泛型容器库，现在就可以开始使用 Go 1.27 的泛型方法了。
